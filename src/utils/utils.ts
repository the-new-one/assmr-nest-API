...
export const checkSubscriptionEveryItemPost = async (userSubscription: Repository<UserSubscription>, userId: number) => {
    ...
    if (subscriptionInfo.userType === 'individual-user') {
        if (!subscriptionInfo.isSubscribed) {
            if (subscriptionInfo.maxNoToPost === 0)
                isCanPost = false;
        }
    }
        ...
        const d2 = new Date();
        if (!subscriptionInfo.isSubscribed || d1 < d2)
            isCanPost = false;
    }
